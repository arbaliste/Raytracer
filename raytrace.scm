...
(define (color-create r g b)
  (vec-create (max (min r 1) 0)
              (max (min g 1) 0)
              (max (min b 1) 0)
              ))
(define (sky-color ray)
  (color-create
   (+ .769 (* (- .000 .769) (/ (+ (vec-y (ray-orig ray)) (* 250 (vec-y (ray-dir ray)))) (screen_height))))
   (+ .510 (* (- .196 .510) (/ (+ (vec-y (ray-orig ray)) (* 250 (vec-y (ray-dir ray)))) (screen_height))))
   (+ .059 (* (- .384 .059) (/ (+ (vec-y (ray-orig ray)) (* 250 (vec-y (ray-dir ray)))) (screen_height))))))
  ...
  (reduce append (list
    (list ; Normal objects
      (sphere-create ball-radius (vec-create 0 0 0)
        (material-create (make-constant-color vec-zero) (vec-create 0.9 0.9 0.9)  (vec-create 1 1 1) 1.3))
